...
if ($login) {
	$id = $_GET["id"];
	$tab = $_GET["tab"];
	$sql = "select * from Vista_Publicaciones_Usuario where id_elemento = $id";
	...
	if ($query != false) {
		...
		<div class="modal-footer">
			<?php if (!$ingeniero) { ?>
				<button class="btn btn-primary d-inline-flex align-items-center" onclick="showEdit(this);" data-fila="<?php echo $id; ?>" data-fun="<?php echo $fun; ?>" type="button">Comprar <i class="bi bi-cart"></i><i class="bi bi-pencil"></i></button>
			<?php } ?>
		</div>
	<?php } else { ?>
